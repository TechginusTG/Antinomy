...
const useFlowStore = create((set, get) => ({
    ...
    save: () => {
        ...
        // --- New: Prompt user for a filename ---
        ...
        // Use default name if input is empty, otherwise use user's input
        const filename = userInput.trim() === "" ? defaultName : userInput.trim();

        // Sanitize the filename and ensure it ends with .json
        const sanitizedFilename = filename.replace(/[\\/:*?"<>|]/g, '_').replace(/\.json$/i, '');
        const finalFilename = `${sanitizedFilename}.json`;
        // --- End of new part ---
