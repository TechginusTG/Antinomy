...
router.put('/settings', authenticateToken, async (req, res) => {
    try {
        ...
        const filteredSettings = Object.entries(settingsToUpdate).reduce((acc, [key, value]) => {
            if (value !== undefined) { // undefined만 제외 (null은 저장 가능하도록)
                acc[key] = value;
            }
            return acc;
        }, {});
...
router.post('/import-request', authenticateToken, async (req, res) => {
    ...
    if (!diagramData || !Array.isArray(chatHistory) || !conversationId) {
        return res.status(400).json({ message: 'diagramData, chatHistory (array), and conversationId are required.' });
    }
    ...
    try {
        ...
        const existingDiagram = await trx('diagrams').where({ user_id: userId }).first();
        if (existingDiagram) {
            await trx('diagrams').where({ user_id: userId }).update({ diagram_data: safeEncodedData, updated_at: db.fn.now() });
        } else {
            await trx('diagrams').insert({ user_id: userId, diagram_data: safeEncodedData });
        }
